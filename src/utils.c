...
SDL_FPoint Pos_world_to_screen(Position target, Camera* cam) {
	...
	double screen_x = (double) diff.x / DEFAULT_MINORGRID_FIXED_POINT;
	double screen_y = (double) diff.y / DEFAULT_MINORGRID_FIXED_POINT;

	screen_x += target.x_maj * DEFAULT_MAJORGRID_CELLSIZE;
	screen_y += target.y_maj * DEFAULT_MAJORGRID_CELLSIZE;

	return (SDL_FPoint) {screen_x + (DEFAULT_MAJORGRID_CELLSIZE >> 1),
	                     screen_y + (DEFAULT_MAJORGRID_CELLSIZE >> 1)};
}
...
/* Sets the length of a vector to FIXED_POINT without sacrificing its rotation
 */
Vector2 Vec2_normalize(Vector2 a, uint32_t fixed_point) {
...
/* Gets the angle to a vector2 from a certain vector2 on a 360 DEGREEE CLOCKWISE
 * circle with NORTH-UP (0° at -y) */
